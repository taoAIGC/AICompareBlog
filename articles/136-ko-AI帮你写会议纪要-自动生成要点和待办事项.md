# AI帮你写会议纪要 - 自动生成要点和待办事项

> 언어: 한국어 | 지역: 글로벌 | 키워드: AI 회의록, ChatGPT, Claude, 회의 메모, AI 효율성

**키워드**: AI 회의록, 회의록, ChatGPT, Claude, DeepSeek, 회의 효율성, AI 업무툴, AI 회의 요약, 자동 생성 회의록, 비즈니스 효율성

---

## 회의 5분, 정리 2시간?

"이 회의 너무 길었는데..."

"아까 논의한 핵심 내용이 뭐였지?"

"누가 뭘 담당하는 거야?"

회의는 업무에서 가장 흔한 일이지만, 회의록을 정리하는 건 회의 자체보다 더 힘들어요.

저는 예전에 회의가 제일 싫었어요. 토론 자체가 무서운 게 아니라 회의 끝나고 나면 하는 일이 더 많아서요. 한 시간짜리 회의에 회의록 정리하느라 한 시간 반을 보내는 경우도 있었어요.

나중에 AI한테 정리 도와달라고 해봤더니, 되게 잘 작동하더라고요. 오늘 그 방법 공유할게요. 여러분도 야근 좀 덜 했으면 좋겠습니다.

---

## 왜 AI가 회의록 정리가 잘 돼?

전통적인 회의록 정리 방식의 고통을 말해보면:

**첫 번째: 기억이 안 나**
회의 중에 정보량이 너무 많아요. 이 사람이 말하고 저 사람이 말하다 보면, 제가 발언할 때쯤 되면 아까 누가 뭐라고 했는지 거의 잊어버려요.

**두 번째: 핵심을 못 잡아**
明明很重要的决定，说的时候大家都在点头，事后没人记得。

**세 번째: 할 일 누락**
"这个你来负责""好的"，然后，就没有然后了。两周后问起来，双方都说以为对方做了。

AI가 이런 문제를 해결해줄 수 있어요:
- 음성 텍스트 변환,发言人 자동 인식
- 핵심 관점 추출, 중요도 구분
- 할 일 자동 정리, 담당자 명확히
- 구조화된 회의록 생성, 한눈에 보기 편하게

---

## 도구 선택

현재主流 AI 회의록 도구들은 다음과 같은 종류가 있어요:

**첫 번째: 전문 회의 도구自带 기능**
- **Otter.ai**:开会的时候实时转录，标记发言人，自动生成摘要。英文做得特别好，中文也在不断优化。
- **飞书妙记**:飞书开会的话直接用，集成度高，免费额度够日常用。
- **腾讯会议**:自带の纪要功能，腾讯会议用户首选。

**두 번째: 범용 AI 도구**
- **ChatGPT/Claude**:把录音或文字稿丢给它，让它帮忙整理。适合已经有文字记录的情况。
- **DeepSeek**:最近用的人越来越多，效果跟ChatGPT差不多，关键是免费。

**세 번째: 로컬 배포**
- **Whisper**:开源の音声→文字モデル、自分で実行、データ漏洩不用担心。企业内网环境用这个比较放心。

---

## 구체적인 사용 방법

### 방법 1: 회의 도구의 AI 기능을 바로 사용

飞书妙记를 예로 들면:

1. 회의 시작 전에 "转写 시작" 클릭
2. 회의 끝나면 "智能会议记录" 클릭
3. 자동으로 생성됨:
   - 회의 요약 (2-3段落로 요약)
   - 챕터별 보기 (시간별로 구분)
   - 발언 요약 (각 사람의 핵심 관점)
   - 할 일 (自动提取)

基本上60分の議事録 уже出来了。想要90分的话，自己再改一改。

### 방법 2: 먼저 전사하고 AI에게 정리 요청

如果你的会议工具没有AI功能，或者转录效果一般，可以这样操作：

**步骤1: 텍스트 자료 확보**
- 用手机录个音，回去转成文字
- 或者用讯飞听见、网易见外这些工具转录

**步骤2: AI에게 정리 지시**

我常用的提示词是这样的：

```
请帮我整理一份会议纪要，内容如下：

【粘贴会议记录】

请按以下格式整理：
1. 会議基本信息（時間、参加者、テーマ）
2. 讨论の主要議題（ポイントごとに列出）
3. 关键結論和決定（突出表示）
4. 待办事項（誰负责，什么时候完成，都要列清楚）
5. 争议のある点（如果有的话）

注意事项：
- 区分清楚每个人的观点
- 待办事項要具体到责任人和截止时间
- 用简洁明了的语言，不要太正式
```

这个提示词基本能产出80分的纪要。

---

## 실전 예시: 제품 요구사항 검토 회의

假设开了个会，讨论要不要做一个新機能。原始会議记录大概是这样的：

---

**会议记录:**

产品经理:大家好，今天讨论一下要做会员体系的事。

技术负责人老王:会员体系我们之前调研过，工作量不小。

产品经理:这个需求确实拖了很久了，客户那边催得紧。

老王:我也知道急，但你看我们现在人手，三个项目并行，人都抽不出来。

产品经理:那你看怎么办？

老王:要不然这样，先上个简单点的，把基础功能做了，增值服务后面再加。

产品经理:也行。那基础功能有哪些？

老王:积分、等级、这几个是最基础的。

产品经理:那这个版本大概多久？

老王:乐观估计一个月。

产品经理:一个月可以接受。那谁负责？

老王:小李牵头，小张配合。

小李:好的。

产品经理:好，那今天先这样，我回去出详细需求。

---

**AI整理后的纪要:**

---

### 产品需求评审会纪要

**会议时间**: 2024年X月X日
**参会人**: 产品经理、技术负责人老王、小李、小张
**会议主题**: 会员体系需求评审

**讨论议题**:

1. 会员体系要不要做?
   - 结论: 做，但分两期
   - 理由: 客户催得紧，但技术资源有限

2. 第一期做什么?
   - 结论: 基础功能（积分、等级）
   - 增值服务（兑换、专属权益）放到二期

**关键决策**:

- 第一期会员体系一个月内完成
- 由小李牵头，小张配合
- 产品经理负责本周内输出详细需求文档

**待办事项**:

| 事项 | 负责人 | 截止时间 |
|------|--------|----------|
| 输出详细需求文档 | 产品经理 | 本周五 |
| 技术方案设计 | 老王 | 下周一 |
| 开始开发 | 小李、小张 | 下周二 |

---

这比看原始记录舒服多了吧?

---

## 심화技巧

### 技巧1: AI에게 "讨论"와 "决策" 구분 요청

很多人写纪要的一大问题是把讨论过程和最终结论混在一起。

可以在提示词里加一句:

> "请明确区分'讨论中提到的观点'和'最终达成的决策'，用不同格式标注"

### 技巧2: 추가 정보 요청

如果原始记录里有些地方写得很模糊，可以让AI追问。

比如原记录写着"小李负责"，可以让AI自动补全:

> "请根据上下文推断每个待办事项的责任人和截止时间，如果信息不足，用【待确认】标注"

### 技巧3: 여러 버전 생성

如果这封纪要要发给不同的人，可以让AI出不同版本:

> "请生成三个版本的纪要:
> 1. 完整版（发给参会人员）
> 2. 精简版（发给领导，只看结论和待办）
> 3. 执行版（只发给相关执行人）"

### 技巧4: 할 일 달력에导入

很多AI工具支持把提取出的待办事项导出为日历事件或任务。

以ChatGPT为例，生成纪要后可以让它:

> "把上面的待办事项生成iCalendar格式的文本"

然后复制到日历里，到期自动提醒.

---

## 자주 묻는 질문

### Q1: 회의录音 못 하면怎么办?

有些会议涉及敏感信息，不让录音。那就自己勤快点，边听边记要点。

我的方法是:
- 只记关键词，谁说了什么观点
- 用#标记决策，用!标记待办
- 散会后马上整理，趁还记得的时候

### Q2: AI整理的纪要不准确怎么办?

检查几个常见问题:

1. **转录错误**:先确认原始转录是否正确，有时候同音字会闹笑话
2. **上下文缺失**:有些会议是接续开的，AI不知道前面的背景，可能理解偏
3. **专业术语**:行业黑话AI可能不认识，可以提前给AI一份术语表

### Q3: 人多会议分不清谁是谁?

最好在会议开始前大家先自报家门，或者主持人介绍一圈。

如果转录工具支持标记发言人，让它自动标注。不支持的话，在提示词里加一句:

> "请根据上下文判断这段话是谁说的，如果不确定就标注【未知发言人】"

### Q4: 会议太长怎么办?

拆开整理.

几个小时的会议，不可能一次整理完。我通常这样做:

1. 每小时或每结束一个议题，生成一个小结
2. 最后把这些小结汇总成完整版
3. 也可以让AI先读一遍整体结构，再让它深入整理每个部分

---

## 권장 워크플로우

结合我自己的使用经验，推荐一个高效的工作流:

**会前**:提前拿到议程，知道要讨论什么，心里有底

**会中**:
- 用飞书/Otter开实时转写
- 边听边在脑子里过一遍重点
- 对重要决策当场确认:"我总结一下，刚才说的是不是这样..."

**会后**:
- 第一时间打开转写记录，修正明显错误
- 丢给AI，按提示词整理
- 自己过一遍，补上AI漏掉的细节
- 确认待办事项有没有漏、责任人有没有写错
- 发出纪要，同时抄送相关leader

整个流程下来，一个小时左右的会，整理时间控制在15-20分钟.

---

##总结

会议纪要的核心是**信息传递**和**责任明确**.

用AI不是偷懒，是把重复劳动交给机器，让自己专注在真正需要思考的事情上.

但AI终究是工具，关键点还得自己把控:
- 决策有没有记对?
- 待办有没有漏人?
- 时间节点有没有写错?

这些涉及到人和时间的事，AI只能辅助，不能全信.

今天分享的方法，希望能帮你省点时间少加会儿班.有问题随时交流.

---

**相关搜索**: AI 회의록, 회의록, ChatGPT, Claude, DeepSeek, 회의 효율성, AI 업무툴, AI 회의 요약, 자동 생성 회의록, 비즈니스 효율성, AI 음성 인식 회의록, AI 회의 어시스턴트